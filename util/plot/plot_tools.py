...
def plot_field(field, 
    ...
    '''
    #TODO: how to  make that the zoom has the same as the others? And debug, mask inside surface??? 
   #recurrent and self_called property

    # #1.Initialize the option variables == Default values
    # edges=None
    # square=None
    # circle=None
    # aux_contourn=False
    ...
    #     x,y,X,Y,cx,cy = get_grid(field)
    # else:
    #     x,y,X,Y,cx,cy = get_grid(field,Lx=Lx,Ly=Ly,dx=dx,dy=dy)
    ...
    if 'surface' in plot_type:
        ...
        for op in options:
            globals()[op[0]] = op[1]   #TODO: locals() gives only for? try to solve using locals to avoid conflicst in globals
        ...
        if edges is not None if 'edges' in globals() else False:
            ...
            ax.plot(x[edge_vt_x], y[edge_vt_y], 'Pk', markersize=7)   #TODO: size as function of dx, etc
        ...
        if square is not None if 'square' in globals() else False:
            ...
            #ax.plot(square[0], square[2],'bo')
            #ax.plot(square[1], square[3],'go')
...
def plot_distribution(field, edges,
    ...
    if 'up_down' in plot_type:
        ...
        #TODO: si el NX no es exacto... el angle i edge dan tamaños 99 senares y entonces problema

        #Plot Line
        #ax.plot(angle[np.where(angle<=180)]
        #    int(len(edge_x)/2):], Z[edge_x[:int(len(edge_x)/2)],edge_y[:int(len(edge_x)/2)]], '--k')
        ...
        #TODO: verify that the seconf part angles are exactly the same as the first + - 360 ....
...
class GIF():
    ...
    #TODO: if type of plot inside add frame
...
def plot_snapshots(ite,field,title, dt, Lx, Ly, dx, dy):
    ...
    #imshow(mpimg.imread('./results_cfl_0.2/A_1.5_RE_100.0_dx_448_448_pressure_timestep_100.png'))
...
def plot_field_old(Lx, Ly, dx, dy, 
    ...
    #TODO: arreglar opcions plots, i estructura interna ['plot',[ ['plot options' , xx], ['plot options' , xx] ]]
  


    #field.dx  if grid field
    #else if numpy field .shape etc 
    ...
    #Convert Grid objects into numpy arrays    #TODO: check if passed grid, numpy, etc
    if isinstance(field,StaggeredGrid):
    ...
    if 'mask' in plots:
        ...
        #la mask per establir colors si no no caldria

             
        
        plt.pcolormesh(X,Y,mask, vmin=0, vmax=5, shading='auto')
        #plt.pcolormesh(X,Y,mask_inverse, vmin=0, vmax=5, shading='auto')
        ...
        #ax.plot(X[np.where(Z==1)],Y[np.where(Z==1)],'rx')


        #y3 = np.ma.masked_where(y > 0.7, y)   #aixo per eliminar algun field
        # z = np.ma.array(z, mask=mask)

        #TODO: adaptar (if) en cas de mask de numpy
    
    #Probe plot
    #TODO: passar com a mascara i agafar sols cantonades
    #ax.plot([xp1, xp2, xp2, xp1, xp1],[yp1, yp1, yp2, yp2, yp1], 'r--', linewidth=2)
    ...
    #Surface plot
    if 'surface' in plots:
        #cs = ax.contourf(X, Y, Z) #, np.arange(-0.3, 0.3, .005), extend='neither') #, corner_mask=False)
        ...
        # #TODO: define as full_zoom option
        ...
        #im = plt.pcolormesh(X[x1:x2,y1:y2],Y[x1:x2,y1:y2],Z[x1:x2,y1:y2], cmap=cmap, norm=norm, shading='auto') #ESTAN INVERTITS!!!!!!!!!!!!! perk abans anava??
        im = plt.pcolormesh(X[y1:y2,x1:x2],Y[y1:y2,x1:x2],Z[y1:y2,x1:x2], cmap=cmap, norm=norm, shading='auto') #perk en Z tmb si esta rot90??
        

        #im = plt.pcolormesh(X,Y,Z, cmap=cmap, norm=norm, shading='auto')
        #cbar = fig.colorbar(im)
        #cbar.ax.set_ylabel(lbar)

        #cs = ax.contour(X, Y, Z, colors='k', alpha=0.5, linestyles='dashed')
        #ax.clabel(cs, colors='k', inline=True, fontsize=10)
        ...
        # ax.plot([vertices[0], vertices[1], vertices[1], vertices[0], vertices[0]], [vertices[2], vertices[2], vertices[3], vertices[3], vertices[2]] ,'r-')
        # ax.plot(vertices[0], vertices[2],'bo')
        # ax.plot(vertices[1], vertices[3],'go')
        ...
        if 'zoom' in plots:
            ...
            ##
            axins.pcolormesh(X,Y,Z, cmap=cmap, norm=norm, shading='auto')
            #cs = axins.contour(X, Y, Z, colors='k', alpha=0.5, linestyles='dashed')
            #axins.clabel(cs, colors='k', inline=True, fontsize=10)
...
#TODO: por ahora los plot velocity y spectrum en simulation, hacer que el esqueleto sea el de aqui, y passar de simula
