...
def setWallBcs(U, flags):
    ...
    obst100 = flags[idx_b, zero, k, j, i_l].eq(CellType.TypeObstacle).__and__(mCont_b)
    ...
    obs_fluid100 = (flags[idx_b, zero, k, j, i_l].eq(CellType.TypeFluid)). \
     __and__(cur_obs).__and__(mCont_b)
    ...
    obst010 = (flags[idx_b, zero, k, j_l, i].eq(CellType.TypeObstacle)).__and__(mCont_b)
    ...
    if (is3D):
        ...
        obst001 = zeroBy.where( k <= 0, (flags[idx_b, zero, k_l, j, i].eq(CellType.TypeObstacle))).__and__(mCont_b)
        U[:,2].masked_fill_(obst001, 0)

        obs_fluid001 = zeroBy.where( k <= 0, (flags[idx_b, zero, k_l, j, i].eq(CellType.TypeFluid))). \
        _and__(cur_obs).__and__(mCont_b)
