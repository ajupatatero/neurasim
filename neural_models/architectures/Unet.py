...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        conv_5_event.record()
        torch.cuda.synchronize()
        conv_6in_cat = torch.cat((F.interpolate(convN_5out,scale_factor=2,mode = 'bilinear', align_corners= False), convN_4out),dim = 1) 
        conv_6_event.record()
        torch.cuda.synchronize()
        convN_6out = self.convN_6(conv_6in_cat)
        conv_7_event.record()
        torch.cuda.synchronize()
        conv_7in_cat = torch.cat((F.interpolate(convN_6out,scale_factor=2,mode = 'bilinear', align_corners= False), convN_3out),dim = 1) 
        conv_8_event.record()
        torch.cuda.synchronize()
        convN_7out = self.convN_7(conv_7in_cat)
        conv_9_event.record()
        torch.cuda.synchronize()
        conv_8in_cat = torch.cat((F.interpolate(convN_7out,scale_factor=2,mode = 'bilinear', align_corners= False), convN_2out),dim = 1)
        conv_10_event.record()
        torch.cuda.synchronize()
        convN_8out = self.convN_8(conv_8in_cat)
        conv_11_event.record()
        torch.cuda.synchronize()
        conv_9in_cat = torch.cat((F.interpolate(convN_8out,scale_factor=2,mode = 'bilinear', align_corners= False), convN_1out),dim = 1)
